...
namespace DAL
{
    public class RevenueDAL
    {
        ...
        {
            string query = @"select count(ord.orderID) as 'Total_order', sum(ort.quantity) as 'Amount', sum(ort.quantity * ds.unit_price) as 'Revenue_of_day' from Dish ds
        ...
        {
            string query = @"select count(ord.orderID) as 'Total_order', sum(ort.quantity) as 'Amount', sum(ort.quantity * ds.unit_price) as 'Revenue_of_month' from Dish ds
                             inner join OrderDetail ort on ds.dishID = ort.dishID
                             inner join Orders ord on ort.orderID = ord.orderID
                             inner join Salesmans sl on ord.salesmanID = sl.salesmanID
                             inner join Shop s on sl.salesmanID = s.shopID
